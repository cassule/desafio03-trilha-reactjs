...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      const updatedProductCart = [...cart];
      const productExist = updatedProductCart.find(product => product.id === productId);
      ...
      if(productExist) {
        ...
        updatedProductCart.push(newProductCart);
      }
      
      setCart(updatedProductCart);
      localStorage.setItem('@RocketShoes:cart', JSON.stringify(updatedProductCart));
